...
class CaptureHervelvetvase(CaptureBase):
    home_url = 'http://www.hervelvetvase.com/'
    HEADER = '''
            ...
            User-Agent:{}
            '''
    Channel = 'hervelvetvase'
    def __init__(self, user_agent, proxy_ip=None):
        super(CaptureHervelvetvase, self).__init__(user_agent, proxy_ip)
        self.header = self._getDict4str(self.HEADER.format(self.user_agent))

    def __del__(self):
        super(CaptureHervelvetvase, self).__del__()
    '''
    ...
    def dealHomeGoods(self):
        ...
        try:
            ...
            pre_load_data = soup.find('div', {'class': 'banner_items clearfix'}).findAll('div')
            for load_data in pre_load_data:
                ...
                href = load_data.find('a').attrs['href']
                if not href.startswith('http://'):
                    href = urljoin(self.home_url, href)
                resultData['LINK'.lower()] = href
...
def main():
    ...
    objCaptureHervelvetvase = CaptureHervelvetvase(useragent)
    objCaptureHervelvetvase.dealHomeGoods()
