...
class CaptureLovebonito(CaptureBase):
    home_url = 'https://www.lovebonito.com/'
    HEADER = '''
    ...
    def __init__(self, user_agent, proxy_ip=None):
        super(CaptureLovebonito, self).__init__(user_agent, proxy_ip)
        self.header = self._getDict4str(self.HEADER.format(self.user_agent))

    def __del__(self):
        super(CaptureLovebonito, self).__del__()
    '''
    ...
    def dealHomeGoods(self):
        ...
        try:
            ...
            pre_load_data = soup.findAll('div', {'class': 'banner-item'})
            for load_data in pre_load_data:
                ...
                try:
                    ...
                    resultData['TITLE'.lower()] = load_data.find('img').attrs['title']
...
def main():
    ...
    objCaptureLovebonito = CaptureLovebonito(useragent)
    objCaptureLovebonito.dealHomeGoods()
